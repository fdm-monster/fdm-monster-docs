...
If you want to run the FDM Monster Vue project (client) as well, please refer to the **[Client Setup](./setup_client.mdx)** guide.
:::
...
1) Launch the FDM Monster server:
    ...
    1) VS Code: Create a run configuration that executes `yarn run dev` as the command. (See [VS Code Run Configuration](#vs-code-run-configuration) for detailed steps.)
    1) WebStorm: Create a run configuration that executes `yarn run dev` as the command. (See [WebStorm Run Configuration](#webstorm-run-configuration) for detailed steps.)
...
The next step is to set up the FDM Monster client. Please refer to the [Client Setup](./setup_client.mdx) guide.
