...
3. Fill in a name for the new API key
4. Click "Create API Key"
5. Provide a name for the key (e.g., "My Integration Script")
...
curl -H "Authorization: Bearer YOUR_API_KEY" \
  http://localhost:4000/api/v2/api-keys
```

This returns a list of your keys (without exposing the secret tokens).
...
curl -X DELETE \
  ...
  http://localhost:4000/api/v2/api-keys/KEY_ID
```

Replace `KEY_ID` with the ID of the key you want to revoke.
