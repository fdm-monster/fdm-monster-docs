---
title: Using mDNS enabled docker image
...
FDM Monster provides container images with mDNS support. mDNS support requires use of the Docker network driver macvlan. This limits support to hosts running a Linux operating system.
...
- The default gateway of for the host interface.
...
The current FDM Monster image does not support DCHP address assignment. If you do not assign a static IP from the expected range, Docker will use the "next, unassigned IP" from the range provided to the bridge network. It is highly recommended that you assign a static IP address to the container **or** provide a hostname for the container (enabling mDNS lookup).
